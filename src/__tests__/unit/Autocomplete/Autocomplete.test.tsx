...
describe('Autocomplete', () => {
  ...
  it('Arranges Autocomplete end adornment items in the expected order (Clear -> Custom -> Error -> Popup -> FixedIconButton)', () => {
    configure({ testIdAttribute: 'data-mui-test' });
    const { container } = render(
      <Autocomplete
        options={['Apple', 'Banana']}
    ...
    // Query the actual DOM nodes generated by MUI and our TextField
    const clearNode = container.querySelector('.MuiAutocomplete-clearIndicator');
    const customNode = screen.getByText('Custom');
    const warningIconNode = screen.getByTestId('warningIcon');
    const popupNode = container.querySelector('.MuiAutocomplete-popupIndicator');
    const fixedButtonNode = screen.getByRole('button', { name: 'fixed-button' });

    // Ensure the native MUI icons were successfully rendered
    expect(clearNode).not.toBeNull();
